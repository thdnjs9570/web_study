...
<form ng-submit="onSubmit($event)" ng-controller="formCtrl" action="signup.do" method="post" 
	...
	<input type="text" name="id" id="id" ng-model="id" ng-change="onIdInput()"/>
...
<script>
	...
	app.controller("formCtrl",function($scope,$http){//ajax기능을 가지고있는 서비스 객체:$http
		
		$scope.onIdInput = function(){
			//입력한 내용을 서버에 ajax요청을 통해서 보낸다.
			$http({
				url:"checkid.do",
				method:"get",
				params:{inputId:$scope.id}//get 방식 요청 parameter 를 적는것이다 단) post는 좀 다르다
